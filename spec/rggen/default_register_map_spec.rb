...
RSpec.describe RgGen::DefaultRegisterMap do
  ...
  describe '既定セットアップ' do
    it 'フィーチャーの読み出しと有効化を行う' do
      ...
      expect(builder).to receive(:enable).with(:bit_field, :type, [:rc, :reserved, :ro, :rof, :rs, :rw, :rwc, :rwe, :rwl, :rws, :w0c, :w0crs, :w0s, :w0src, :w0t, :w0trg, :w1, :w1c, :w1crs, :w1s, :w1src, :w1t, :w1trg, :wo, :wo1]).and_call_original
