      ...
      context '入力がwidth_first/lsb_first以外の場合' do
        it 'SourceErrorを起こす' do
          ['foo_first', 'width_foo', 'lsb_foo', 'width_lsb', true, false, nil, 1, Object.new].each do |value|
            ...
            }.to raise_source_error "illegal input value for bit assignment: #{value.inspect}"
          end
        end
      end
    end
  ...
  describe 'エラーチェック' do
    context 'ビット割当が未入力の場合' do
      it 'SourceErrorを起こす' do
        expect {
          create_bit_field
        }.to raise_source_error 'no bit assignment is given'
    ...
    context 'LSB または WIDTH が入力されなかった場合' do
      it 'SourceErrorを起こす' do
        ...
        }.to raise_source_error 'neither lsb nor width is given'
      end
    end
    ...
    context '入力LSBが整数に変換できない場合' do
      it 'SourceErrorを起こす' do
        [nil, true, false, 'foo', '0xef,_gh', Object.new].each do |invalid_value|
          ...
          }.to raise_source_error "cannot convert #{invalid_value.inspect} into bit assignment(lsb)"
        end
      end
    end
    ...
    context '入力LSBが 0 未満の場合' do
      it 'SourceErrorを起こす' do
        [random_value(-8, -3), -2, -1].each do |invalid_value|
          expect {
            create_bit_field(lsb: invalid_value)
          }.to raise_source_error "lsb is less than 0: #{invalid_value}"
        end
      end
    end
    ...
    context '入力ビットフィールド幅が整数に変換できない場合' do
      it 'SourceErrorを起こす' do
        [nil, true, false, 'foo', '0xef,_gh', Object.new].each do |invalid_value|
          ...
          }.to raise_source_error "cannot convert #{invalid_value.inspect} into bit assignment(width)"
        end
      end
    end
    ...
    context '入力ビットフィールド幅が 1 未満の場合' do
      it 'SourceErrorを起こす' do
        [random_value(-8, -2), -1, 0].each do |invalid_value|
          expect {
            create_bit_field(lsb: 0, width: invalid_value)
          }.to raise_source_error "width is less than 1: #{invalid_value}"
        end
      end
    end
    ...
    context '入力繰り返し数が整数に変換できない場合' do
      it 'SourceErrorを起こす' do
        [nil, true, false, 'foo', '0xef,_gh', Object.new].each do |invalid_value|
          ...
          }.to raise_source_error "cannot convert #{invalid_value.inspect} into bit assignment(sequence size)"
        end
      end
    end
    ...
    context '入力繰り返し数が 1 未満の場合' do
      it 'SourceErrorを起こす' do
        [random_value(-8, -2), -1, 0].each do |invalid_value|
          expect {
            create_bit_field(lsb: 0, sequence_size: invalid_value)
          }.to raise_source_error "sequence size is less than 1: #{invalid_value}"
        end
      end
    end
    ...
    context '入力繰り返し幅が整数に変換できない場合' do
      it 'SourceErrorを起こす' do
        [nil, true, false, 'foo', '0xef,_gh', Object.new].each do |invalid_value|
          ...
          }.to raise_source_error "cannot convert #{invalid_value.inspect} into bit assignment(step)"
        end
      end
    end
    ...
    context '入力繰り返し幅がビット幅より小さい場合' do
      it 'SourceErrorを起こす' do
        ...
        [0, -1, random_value(-8, -3)].each do |step|
          expect {
            create_bit_field(lsb: 0, sequence_size: 1, step: step)
          }.to raise_source_error "step is less than width: step #{step} width 1"
        end
        ...
        [1, 0, -1, random_value(-8, -2)].each do |step|
          expect {
            create_bit_field(width: 2, sequence_size: 1, step: step)
          }.to raise_source_error "step is less than width: step #{step} width 2"
        end
      end
    end
    ...
    context '割り当てたビットが既出の場合' do
      ...
      it 'SourceErrorを起こす' do
        ...
        }.to raise_source_error 'overlap with existing bit field(s)'
