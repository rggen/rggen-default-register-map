  ...
  context '参照ビットフィールドの指定がある場合' do
    it '不揮発性ビットフィールドである' do
      bit_fields = create_bit_fields do
        register do
          name :foo_0
          bit_field { name :foo_0; bit_assignment lsb: 0; type :rwl; initial_value 0; reference 'foo_1.foo_1' }
        end
        register do
          name :foo_1
    ...
    specify '1ビット幅以上の参照ビットフィールドの指定が必要' do
      expect {
        create_bit_fields do
          register do
            name :foo_0
            bit_field { name :foo_0; bit_assignment lsb: 0, width: 2; type :rwl; initial_value 0; reference 'foo_1.foo_1' }
          end
          register do
            name :foo_1
          ...
          register do
            name :bar_0
            bit_field { name :bar_0; bit_assignment lsb: 0, width: 1; type :rwl; initial_value 0; reference 'bar_1.bar_1' }
          end
          register do
            name :bar_1
    ...
    it 'RegisterMapErrorを起こす' do
      expect {
        create_bit_fields do
          register do
            name :foo
            bit_field { name :foo_0; bit_assignment lsb: 0, width: 1; type :rwl; initial_value 0; reference 'foo.foo_1' }
