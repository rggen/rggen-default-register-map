...
RSpec.describe 'bit_field/type/wrs' do
  ...
  specify 'ビットフィールド型は:wrs' do
    bit_fields = create_bit_fields do
      register do
        name :foo
        bit_field { name :foo; bit_assignment lsb: 1; type :wrs; initial_value 0 }
      end
    end
    expect(bit_fields[0]).to have_property(:type, :wrs)
  end
  ...
  specify '初期値の指定は必要' do
    expect {
      create_bit_fields do
        register do
          name :foo
          bit_field { name :foo; bit_assignment lsb: 1; type :wrs }
        end
      end
