...
RSpec.describe 'bit_field/type/row1trg' do
  ...
  specify 'ビットフィールド型は:row1trg' do
    bit_fields = create_bit_fields do
      register do
        name :foo
        bit_field { name :foo; bit_assignment lsb: 0; type :row1trg }
      end
    end
    expect(bit_fields[0]).to have_property(:type, :row1trg)
  end
  ...
  specify '初期値の指定は不要' do
    expect {
      create_bit_fields do
        register do
          name :foo
          bit_field { name :foo; bit_assignment lsb: 0; type :row1trg; initial_value 0 }
        end
        register do
          name :bar
          bit_field { name :bar; bit_assignment lsb: 0; type :row1trg }
        end
      end
    }.not_to raise_error
  end
    ...
    specify '同一幅のビットフィールドの指定が必要' do
      expect {
        create_bit_fields do
          register do
            name :foo
            bit_field { name :foo_0; bit_assignment lsb: 0, width: 1; type :row1trg; reference 'bar.bar_0' }
            bit_field { name :foo_1; bit_assignment lsb: 1, width: 2; type :row1trg; reference 'bar.bar_1' }
          end
          register do
            name :bar
