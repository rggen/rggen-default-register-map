...
RSpec.describe 'bit_field/type/w0src' do
  ...
  specify 'ビットフィール型は:w0src' do
    bit_fields = create_bit_fields do
      register do
        name :foo
        bit_field { name :foo; bit_assignment lsb: 1; type :w0src; initial_value 0 }
      end
    end
    expect(bit_fields[0]).to have_property(:type, :w0src)
  end
  ...
  specify '初期値の指定は必要' do
    expect {
      create_bit_fields do
        register do
          name :foo
          bit_field { name :foo; bit_assignment lsb: 1; type :w0src }
        end
      end
  ...
  specify '参照ビットフィールドの指定は不要' do
    expect {
      create_bit_fields do
        ...
        register do
          name :bar
          bit_field { name :bar; bit_assignment lsb: 1; type :w0src; initial_value 0; reference 'foo.foo' }
        end
      end
    }.not_to raise_error
  end
