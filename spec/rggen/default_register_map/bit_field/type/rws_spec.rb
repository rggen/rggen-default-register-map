...
RSpec.describe 'bit_field/type/rws' do
  ...
  specify 'ビットフィール型は:rws' do
    bit_fields = create_bit_fields do
      register do
        name :foo
        bit_field { name :foo_0; bit_assignment lsb: 0; type :rws; initial_value 0 }
      end
    end
    expect(bit_fields[0]).to have_property(:type, :rws)
  end
  ...
  specify '初期値の指定が必要' do
    expect {
      create_bit_fields do
        register do
          name :foo
          bit_field { name :foo_0; bit_assignment lsb: 0; type :rws }
        end
      end
    ...
    specify '1ビット幅以上の参照ビットフィールドの指定が必要' do
      expect {
        create_bit_fields do
          register do
            name :foo
            bit_field { name :foo_0; bit_assignment lsb: 0, width: 2; type :rws; initial_value 0; reference 'foo.foo_1' }
            bit_field { name :foo_1; bit_assignment lsb: 8, width: 1; type :rw; initial_value 0 }
          end

          register do
            name :bar
            bit_field { name :bar_0; bit_assignment lsb: 0, width: 1; type :rws; initial_value 0; reference 'bar.bar_1' }
