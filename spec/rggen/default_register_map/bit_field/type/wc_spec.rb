    ...
    specify '同一幅のビットフィールドの指定が必要' do
      expect {
        create_bit_fields do
          register do
            name :foo
            bit_field { name :foo_0; bit_assignment lsb: 0, width: 1; type :wc; reference 'bar.bar_0'; initial_value 0 }
            bit_field { name :foo_1; bit_assignment lsb: 1, width: 2; type :wc; reference 'bar.bar_1'; initial_value 0 }
          end
          register do
            name :bar
