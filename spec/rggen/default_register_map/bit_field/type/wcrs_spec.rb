...
RSpec.describe 'bit_field/type/wcrs' do
  ...
  specify 'ビットフィールド型は:wcrs' do
    bit_fields = create_bit_fields do
      register do
        name :foo
        bit_field { name :foo; bit_assignment lsb: 1; type :wcrs; initial_value 0 }
      end
    end
    expect(bit_fields[0]).to have_property(:type, :wcrs)
  end
  ...
  specify '初期値の指定は必要' do
    expect {
      create_bit_fields do
        register do
          name :foo
          bit_field { name :foo; bit_assignment lsb: 1; type :wcrs }
        end
      end
